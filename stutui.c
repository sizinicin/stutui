/*
 * stutui - A minimalist terminal‐based study timer and stopwatch with daily logging, built on ncurses.
...
void init_log_path() {
  ...
  char config_dir[512];
  snprintf(config_dir, sizeof(config_dir), "%s/.config/stutui", home);
  struct stat st = {
    0
  };
  ...
  snprintf(log_filepath, sizeof(log_filepath), "%s/stutui.log", config_dir);
  snprintf(lock_filepath, sizeof(lock_filepath), "%s/stutui.lock", config_dir);
  snprintf(config_filepath, sizeof(config_filepath), "%s/stutui.conf", config_dir);
...
void draw_minutes(int m, int rows, int cols) {
  char buf[64];
  snprintf(buf, sizeof(buf), "%d minutes studied today", m);
  attron(COLOR_PAIR(5));
...
int main() {
  ...
  if (flock(lock_fd, LOCK_EX | LOCK_NB) != 0) {
    while (1) {
      ...
      const char * msg = "Another instance of stutui is already running.";
